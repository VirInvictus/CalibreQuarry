...
- **Stdlib only.** No `requirements.txt`, no `pip install <thirdparty>`. Allowed: `sqlite3`, `json`, `csv`, `argparse`, `curses`, `os`, `sys`, `shutil`, `tempfile`, `struct`, `re`, `unicodedata`, `datetime`, `typing`, `subprocess`, `io`, `contextlib`. If a task seems to require a third-party library, stop and ask. (The `scripts/` companion tools are also minimal-dependency (uses tqdm), but may shell out to external binaries like Ghostscript.)
