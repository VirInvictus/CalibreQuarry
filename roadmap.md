...
What's done, what's next. Updated as of v3.47.0.
...
- [ ] LOW — docs/taxonomy.example.yaml embeds the personal library path in a public file; README relative LICENSE href + stats.png src render dead/broken on PyPI; CLAUDE.md contract-note sections out of chronological order (3.42→3.41→3.37→cquarry-1.7→3.24→3.33→3.35→3.32, 3.38/3.39 at the bottom); spec §5 script table missing check_pdf/comments_census/db_util; --wing-without-mode fallback drops fmt (md silently plain); TUI menu claims "every read mode" but lacks --format-stats.
...
- [ ] **A neutral constants module for the parallel dest lists**: SINGLE_BOOK_DESTS (writeops), _WRITE_FLAG_DESTS (restrict), and setwrite's set-mode dests keep step by comments and tests alone; one shared module would delete three keep-in-step obligations. (L2.14 polish, deliberately deferred: refactor risk vs blitz window.)
- [ ] **_backup_db is triplicated with divergent exception types** (run.py, integrate.py, setwrite.py): one shared helper would end the drift; the error-mapping choice (ValueError vs _UsageError) needs a decision first.
...
- [ ] **bindery's `manual_watermark_repair` decisions do not mirror into the manifest's decisions_needed** (the sibling gap of the 3.43.0 lossy mirror): books bindery refuses to auto-strip vanish from the durable record.
- [ ] **The lossy-pending flow has an awkward double-manifest wrinkle**: the dry phase-1 records pending repairs, sign consents, and the apply pass is a phase-1 re-run (a NEW manifest under the 3.43.0 unique naming). The unexercised `lossy_consent` decision kind is the designed home for an in-manifest resolution.
