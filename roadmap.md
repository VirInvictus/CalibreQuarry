...
What's done, what's next. Updated as of v3.50.0.
...
- [x] MED — Two unrecorded raw-SQL reads in the frontend (frontend-only-split class): _precedent_tags (run.py:787-805, a 4-table JOIN belonging in cquarry) and _remove_book_dry_run (writeops.py:457-478); fts.py:85 is the only recorded exception. *(3.44.0: _precedent_tags promoted to cquarry 1.22's CalibreDB.precedent_tags under grant #117 (released, on PyPI) and consumed with the floor bump; _remove_book_dry_run converted to CalibreDB reads; both pinned end to end. fts.py's sidecar read remains the one recorded exception.)*
- [x] MED — Comment/header truth batch: --fields help advertises comments which backfill refuses; validate_metadata header omits 3 of its 14 checks; librarything.py header documents a standalone CLI that does not exist; integrate's skip comment swallows no-source rows as already-so (unconvertible books vanish from the apply report, counters zero); SINGLE_BOOK_DESTS omits add_tag/remove_tag; export/catalog annotated -> None but return load-bearing exit codes; reconcile_file_metadata's pgrep is fail-open while its own comment promises the safe answer (the family now has three OSError semantics). *(3.44.0: all seven sites truthed -- the --fields help, the validate header's missing three checks, the librarything header rewritten for the real surface, the convert skip comment names both shapes, SINGLE_BOOK_DESTS completed with the append-list dests, the export/catalog -> None annotations, and the reconcile pgrep fail-closed.)*
- [x] MED — Publish-path hardening (workspace batch with cquarry + vir-tui, byte-identical conventions): SHA-pin pypa/gh-action-pypi-publish@release/v1 (guards id-token: write), pypi environment has no deployment policy, no tag protection, publish.yml lacks the top-level permissions block, publish path never runs ruff, no concurrency groups. *(3.44.0: SHA pins, permissions split, concurrency, pinned ruff, twine+smoke, and the create-release job shipped here and in cquarry 1.22.0; vir-tui verified as already shipped. The pypi-environment deployment policy was probed and REVERTED -- REST policies are branch-type only and reject tag deployments outright (cquarry's v1.22.0 publish proved it); tag protection landed instead as the release-tags-protected ruleset, and a UI-applied tag policy is the recorded reopen piece.)*
- [x] MED — Docs truth: spec.md:23 floor says >=1.14.0 (third recurrence; real >=1.21.0 — a reader honoring it ships a build where --audit crashes); spec.md:118 --format row missing md; spec.md:223 "no network access" contradicts run backfill; README test-suite section says 373 tests (actual 525) naming 10 of 26 files; version-sync test does not enforce spec/roadmap headers (live proof: roadmap.md:3 still says "as of v3.26.0"). *(3.44.0: the accreted floor sentence collapsed to floor-plus-bumps with the floor pinned against pyproject, the --format row names md, the no-network claim is scoped to the read surface with run backfill named, the README test section says 542/26 naming every suite, and the version-sync test guards the spec header, the spec floor, and the roadmap stamp.)*
- [x] MED — Housekeeping: .gitignore lacks venv/.venv_ci/.pytest_cache/.ruff_cache/.claude (tool self-ignores are masking it); pyproject [dependency-groups] declares pytest+pytest-asyncio with zero references anywhere (pure unittest repo; contradicts the identity line). *(3.44.0: .gitignore gained the five tool dirs, the dev group deleted on Brandon's gate, the taxonomy placeholderd, README links absolutized, REPORT-12-Sept.md moved to docs/.)*
- [x] LOW — docs/taxonomy.example.yaml embeds the personal library path in a public file; README relative LICENSE href + stats.png src render dead/broken on PyPI; CLAUDE.md contract-note sections out of chronological order (3.42→3.41→3.37→cquarry-1.7→3.24→3.33→3.35→3.32, 3.38/3.39 at the bottom); spec §5 script table missing check_pdf/comments_census/db_util; --wing-without-mode fallback drops fmt (md silently plain); TUI menu claims "every read mode" but lacks --format-stats. *(All six: the taxonomy placeholder and README links in 3.44.0, the --wing fmt in 3.43.0, the TUI menu in 3.46.0, and the CLAUDE.md order + spec §5 table in 3.48.0.)*
...
- [x] **The provenance seeder emits `Z-Library`; the ruled enum value is
      ...
      spelling; spec synced). Phase 2 stamps cc6 from the reviewed
      provenance, and enum validation refuses unknown values, so every
...
- [x] **EPUB lossy repair candidates do not record in the manifest's
      ...
      pending), but the manifest wrote `lossy: {flagged: false, repairs:
      []}` for the book. The seal binds the lossy list, so a repair the
      manifest does not carry cannot be consented to by signing, and the
      ...
      runs.)*
### New work noticed in the final blitz (recorded 2026-09-15, not executed)

- [x] **A neutral constants module for the parallel dest lists**: SINGLE_BOOK_DESTS (writeops), _WRITE_FLAG_DESTS (restrict), and setwrite's set-mode dests keep step by comments and tests alone; one shared module would delete three keep-in-step obligations. (L2.14 polish, deliberately deferred: refactor risk vs blitz window.) *(3.48.0: `dests.py` is the one source; the consumers import their slices and test_dests.py pins every member against build_parser.)*
- [x] **_backup_db is triplicated with divergent exception types** (run.py, integrate.py, setwrite.py): one shared helper would end the drift; the error-mapping choice (ValueError vs _UsageError) needs a decision first. *(3.48.0: `backups.make_backup`; the decision is recorded: an in-library --backup-dir is a USAGE problem at every door, so the shared helper raises the stdlib-neutral ValueError and each dispatcher keeps its own usage path.)*
...
- [x] **bindery's `manual_watermark_repair` decisions do not mirror into the manifest's decisions_needed** (the sibling gap of the 3.43.0 lossy mirror): books bindery refuses to auto-strip vanish from the durable record. *(3.48.0: `_mirror_bindery_decisions` mirrors them as `manual_repair` entries, per book, path-matched against the manifest's files.)*
- [x] **The lossy-pending flow has an awkward double-manifest wrinkle**: the dry phase-1 records pending repairs, sign consents, and the apply pass is a phase-1 re-run (a NEW manifest under the 3.43.0 unique naming). The unexercised `lossy_consent` decision kind is the designed home for an in-manifest resolution. *(3.48.0: the dry phase-1 emits `lossy_consent` per flagged file; the reviewer sets `"resolution": "apply"` and re-signs, and phase 2 drives the strips itself, flips the lossy records, and consumes the decisions. All-or-nothing; unresolved still blocks. The --apply-lossy re-run path remains.)*
