...
- **Upgrade**: `--analytics` and `--stats` render over `cquarry.analytics` (`author_stats`, `addition_timeline`, `rating_distribution`); output verified identical on the real library. `author_stats` gained an additive `rated_count` key so the "(N rated)" rendering survives the move.
