...
commands=(
  ...
  "python -m cquarry --search \"rating:>=4 and tags:Fic.SciFi\" --db \"$DB_PATH\" > /tmp/test_search_stdout.txt"
  "python -m cquarry --search \"tags:Fic\" --format json --db \"$DB_PATH\" --output /tmp/test_search.json"
  "python -m cquarry --search \"\" --format csv --db \"$DB_PATH\" --output /tmp/test_search_all.csv"
