...
| **Run verbs (Phase 17)** | `run phase1 DIR`, `run phase2 --manifest F --backup-dir D`, `run phase3 --manifest F [--answer-file A]` | The acquisition pathway as commands: phase 1 vets a downloads directory into an `acquisition-manifest/1` manifest (duplicate screen, DRM audit, PDF/DJVU battery via `scripts/check_pdf.py`, bindery's EPUB slice); phase 2 imports the SIGNED manifest as one transaction (`add_book`, `#source`/`#audience` stamped, tags+rating cleared on the imported ids, metadata downloads whose failures become decisions, resumable); phase 3 curates via prompts or an answer file, drives bindery + reconciliation, re-validates, and emits the batch record |
